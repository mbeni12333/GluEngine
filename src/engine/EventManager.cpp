...
void EventManager::LoadBindings(){
	...
	while(std::getline(bindings, line)){
		...
		std::cout << "Reading from callbacks file : " << callbackName << std::endl;
		Binding* bind = new Binding(callbackName);
		// get all the events required
		while(!keystream.eof()){
			std::string keyval;
			keystream >> keyval;
            std::cout << "Reading from callbacks file : " << callbackName << " keyval = " << keyval << std::endl;
			int start = 0;
			int end = keyval.find(delimiter);
			// you screwed
			if(end == std::string::npos){
                std::cout << "Reading from callbacks file : " << callbackName << " shit : " << keystream.eof() << std::endl;
				/*delete bind;
				bind = nullptr;*/
				break;
			}
		...
		if(!AddBinding(bind)){
		    std::cout << "I'm the mofo , and bind is indeed : " << (bind == nullptr) << std::endl;
			delete bind;
		}
		bind = nullptr;
	}
